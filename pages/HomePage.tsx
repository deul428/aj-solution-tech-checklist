...
// Added missing icons ClipboardCheck and ScanQrCode to the import list
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 flex flex-col h-full">
          <div className="flex items-center justify-between mb-6">
            ...
            {masterData.length > 0 && (
              <button onClick={clearMasterData} className="text-red-500 hover:text-red-600 text-sm flex items-center gap-1 transition-all">
          ...
          {!fileName ? (
            ...
            >
              ...
              <p className="text-gray-600 font-semibold mb-1">마스터 엑셀 파일 업로드</p>
              <p className="text-gray-400 text-sm text-center">장비 정보가 담긴 엑셀 파일을<br/>여기에 드래그하거나 클릭하세요.</p>
            ...
            <div className="flex-1 flex flex-col justify-center items-center bg-green-50 rounded-xl p-8 border border-green-100">
              ...
              <h4 className="text-green-900 font-bold text-lg mb-1 truncate max-w-full">{fileName}</h4>
              <p className="text-green-700">{masterData.length.toLocaleString()}건 로드 완료</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6 flex flex-col h-full">
          <div 
            ...
            className={`p-8 rounded-2xl bg-white shadow-lg border border-gray-100 cursor-pointer group hover:shadow-2xl transition-all flex-1 ${!masterData.length ? 'opacity-50 grayscale pointer-events-none' : ''}`}
          >
            ...
            <p className="text-gray-500 text-sm">관리번호를 입력하여 QR코드가 포함된 정비 체크리스트를 즉시 생성합니다.</p>
          </div>
          ...
          >
            ...
            <p className="text-gray-500 text-sm">카메라로 QR을 스캔하여 현장에서 즉시 자산 정보를 확인하고 실사를 기록합니다.</p>
          </div>
        </div>
      </div>
    </div>
