...
const AuditPage: React.FC<AuditPageProps> = ({ masterData, setMasterData, serviceUrl, selectedSheet }) => {
  ...
  useEffect(() => {
    ...
    const loadOptions = async () => {
      const mapping = await fetchLocationOptions(serviceUrl);
  ...
  const handleCenterSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (val === "custom") {
      ...
      setSelectedCenter("");
      setSelectedZone(""); // 센터가 바뀌면 구역도 초기화
    } else {
  ...
  // 선택된 센터에 해당하는 구역 목록 추출
  const availableZones = selectedCenter && locationMapping[selectedCenter] ? locationMapping[selectedCenter] : [];
  const centerOptions = Object.keys(locationMapping).sort();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div className="p-4 bg-black min-h-[400px] flex items-center justify-center relative">
              ...
              {cameraStatus === 'error' && (
                <div className="absolute inset-0 z-10 bg-gray-900 flex flex-col items-center justify-center text-white p-8 text-center">
                  ...
                  <button onClick={startScanner} className="bg-white text-black px-6 py-2.5 rounded-xl font-black text-sm hover:bg-gray-200 transition-all flex items-center gap-2">
                    <Camera className="w-4 h-4" /> 다시 시도
                  </button>
                </div>
              )}
              {cameraStatus === 'ready' && !errorMessage && (
                <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 text-[11px] font-bold px-5 py-2.5 rounded-full pointer-events-none z-10 border shadow-2xl transition-all duration-300 ${isCoolingDown ? "bg-amber-500 text-white border-amber-400 animate-pulse" : "text-white/80 bg-black/60 backdrop-blur-md border-white/10"}`}>
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-2xl border border-gray-100 h-full min-h-[500px] flex flex-col justify-between">
          ...
          <button
            ...
            className={`w-full flex justify-center items-center gap-3 px-8 py-5 rounded-2xl font-black transition-all shadow-xl active:scale-95 ${auditHistory.length > 0 && !isSyncing ? "bg-blue-600 text-white hover:bg-blue-700 hover:shadow-blue-200" : "bg-gray-100 text-gray-400 cursor-not-allowed shadow-none"}`}
          >
            {isSyncing ? <Loader2 className="w-5 h-5 animate-spin" /> : <CloudUpload className="w-6 h-6" />}
              ...
              <>
                <div className="bg-purple-600 p-10 text-white relative">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="bg-white/20 p-2 rounded-xl"><CheckCircle className="w-5 h-5" /></div>
                ...
                <div className="p-10 space-y-8">
                  ...
                  <div className="flex gap-4 pt-4">
                    <button type="button" onClick={closeScanModal} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-black py-5 rounded-3xl transition-all">취소</button>
                    <button type="button" onClick={confirmAudit} className="flex-[2] bg-purple-600 hover:bg-purple-700 text-white font-black py-5 rounded-3xl shadow-2xl shadow-purple-200 flex items-center justify-center gap-3 transition-all active:scale-95"><CheckCircle className="w-6 h-6" /> 실사 확인</button>
                  </div>
                </div>
              </>
            ) : (
              <div className="p-16 text-center">
                <div className="bg-red-50 w-28 h-28 rounded-full flex items-center justify-center mx-auto mb-10"><XCircle className="w-14 h-14 text-red-500" /></div>
                ...
                <button type="button" onClick={closeScanModal} className="w-full bg-gray-900 text-white font-black py-6 rounded-3xl hover:bg-black transition-all shadow-2xl active:scale-95 text-xl">닫기</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-400">
            <div className="bg-blue-600 p-8 text-white relative">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="bg-white/20 p-2 rounded-xl"><MapPin className="w-6 h-6" /></div>
                <h3 className="text-2xl font-black">실사 위치 선택</h3>
              </div>
              <p className="text-blue-100 text-sm font-medium">선택한 센터의 구역만 목록에 표시됩니다.</p>
            </div>
            ...
            <div className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {!isCustomCenter ? (
                    ...
                    <div className="flex gap-2 animate-in slide-in-from-right-2">
                      <input type="text" autoFocus value={selectedCenter} onChange={(e) => setSelectedCenter(e.target.value)} placeholder="직접 입력 (예: 제주 지점)" className="flex-1 bg-white border-2 border-blue-200 rounded-2xl px-5 py-4 font-black text-gray-900 outline-none shadow-lg shadow-blue-50" />
                      <button onClick={() => { setIsCustomCenter(false); setSelectedCenter(""); setSelectedZone(""); }} className="p-4 bg-gray-100 text-gray-500 rounded-2xl hover:bg-gray-200"><RefreshCcw className="w-5 h-5" /></button>
                    </div>
                  )}
                </div>
                  ...
                  {!isCustomZone ? (
                    <div className="relative group">
                      <select 
                        ...
                        className={`w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 font-black text-gray-900 outline-none focus:ring-4 focus:ring-purple-100 transition-all appearance-none cursor-pointer ${(!selectedCenter && !isCustomCenter) ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                    ...
                    <div className="flex gap-2 animate-in slide-in-from-right-2">
                      <input type="text" autoFocus value={selectedZone} onChange={(e) => setSelectedZone(e.target.value)} placeholder="직접 입력 (예: A-1 구역)" className="flex-1 bg-white border-2 border-purple-200 rounded-2xl px-5 py-4 font-black text-gray-900 outline-none shadow-lg shadow-purple-50" />
                      <button onClick={() => { setIsCustomZone(false); setSelectedZone(""); }} className="p-4 bg-gray-100 text-gray-500 rounded-2xl hover:bg-gray-200"><RefreshCcw className="w-5 h-5" /></button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setShowTransferModal(false)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-black py-4 rounded-2xl transition-all">취소</button>
                <button type="button" onClick={handleConfirmTransfer} disabled={isSyncing || !selectedCenter || !selectedZone} className="flex-[2] bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-2xl shadow-xl shadow-blue-100 flex items-center justify-center gap-3 transition-all active:scale-95 disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed">
                  {isSyncing ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />} 서버에 저장하기
                </button>
              </div>
            </div>
          </div>
